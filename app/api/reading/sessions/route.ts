...
export async function POST(request: Request) {
  try {
    ...
    const passageId = requireString(body.passageId, "passageId");
    const level = parseLevel(body.level);
    const session = await createGenericSession(user.id, "reading", level, { passageId, startedAt: body.startedAt });
